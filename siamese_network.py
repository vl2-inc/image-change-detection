...
def extra_feat(img_path):
        ...
        F = tf.concat([x3,x2,x1,x4,x5],3) #Change to only x1, x1+x2,x1+x2+x3..so on, in order to visualize features from different blocks
        return F
def main():
  if (len(sys.argv))>3:
  ...
  image.img_to_array(image.load_img(img_path1)).shape
